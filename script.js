...
class BlackBox {
    constructor(L, l, nbAtomes){
        this.L = L, this.l = l, this.numAtomes = nbAtomes;
        this.creerGrille(this.L, this.l, this.numAtomes);
        ...
        for (let i=0; i<nbAtomes; i++){
            ...
            for (let x = nouvelAtome.x-1; x<=nouvelAtome.x+1; x++) for (let y = nouvelAtome.y-1; y<=nouvelAtome.y+1; y++) if (this.grille[y][x].statut !== ATOME) {     // Champ de l'atome
                if (x===nouvelAtome.x || y===nouvelAtome.y){
                    ...
                    if (this.grille[y][x].statut === DEVIE || typeof this.grille[y][x].num === "number") this.grille[y][x].statut = REFLECHI;
    ...
    coup(num){
        ...
        while (true) {
            ...
            x+= deplacementX;
            y+= deplacementY;
            if (this.grille[y][x].num) return this.grille[y][x].num;
        }
    }
}
...
function afficherGrille(g){
    ...
    for (let y in g) {
        html+="<tr>";
        for (let x in g[y]){
            html+=`<td class="${(typeof g[y][x].num === "number")? "gris": ""}">`+g[y][x].statut+"</td>";
        }
        html+="</tr>";
    }
    html+= "</table>";
